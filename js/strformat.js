...
var strformat = {
	...
	showqp:function(type,name,dt,cont,nuid, fase,rnd){
		...
		nr+='		<tr valign="top">';
		if(type == 'left'){
			nr+='			<td width="50" align="center"><img src="'+fase+'" class="qipaoface r-face" width="40" height="40"></td>';
	...
	contshozt:function(d, nuid){
		...
		if(js.isimg(d.fileext)){
			sttr='';
			if(d.thumbpath){
				s='<img src="'+d.thumbpath+'" onclick="strformat.openimg(this, '+d.fileid+')">';
			}else{
				if(d.width){
			...
			s='<div onclick="strformat.openfile('+d.fileid+',\''+d.fileext+'\',\''+nuid+'\')"><i class="icon-volume-up"></i> '+(parseInt(d.filesize/1500))+'"</div>';
		}else{
			slx = d.fileext;
			if(js.fileall.indexOf(','+slx+',')<0)slx='wz';
			s=''+d.filename+' <span style="font-size:14px;color:#888888">('+d.filesizecn+')</span>';
			s='<table onclick="strformat.openfile('+d.fileid+',\''+d.fileext+'\',\''+nuid+'\')"><tr><td><div class="qipaofile">'+d.fileext.toUpperCase()+'</div></td><td>'+s+'</td></tr></table>';
		}
		return s;
	},
	openfile:function(){
